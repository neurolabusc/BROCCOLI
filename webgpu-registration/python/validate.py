#!/usr/bin/env python3
"""Validate WebGPU registration against OpenCL reference outputs."""
import sys, os
import numpy as np
...
# Add this directory to path for the webgpu_registration module
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

import webgpu_registration
...
WEBGPU_OUT_DIR = os.path.join(REGISTER_DIR, 'webgpu_outputs')
...
def validate_epi_t1():
    ...
    aligned, interpolated, params = webgpu_registration.registerEPIT1(
    ...
    if os.path.exists(ref_path):
        ...
        print(f"  NCC (WebGPU vs OpenCL reference): {ncc_ref:.4f}")

    # Save outputs
    os.makedirs(WEBGPU_OUT_DIR, exist_ok=True)
    t1_ref_path = os.path.join(REGISTER_DIR, 't1_brain.nii.gz')
    save_nifti(aligned_unpacked, t1_ref_path,
               os.path.join(WEBGPU_OUT_DIR, 'epi_t1_aligned.nii.gz'))
    interp_unpacked = unpack_volume(interpolated, t1_ref.shape)
    save_nifti(interp_unpacked, t1_ref_path,
               os.path.join(WEBGPU_OUT_DIR, 'epi_t1_interpolated.nii.gz'))
    np.savetxt(os.path.join(WEBGPU_OUT_DIR, 'epi_t1_params.txt'), params)
...
def validate_t1_mni(res='2mm'):
    ...
    t0 = time.perf_counter()
    results = webgpu_registration.registerT1MNI(
    ...
    for label, vol, ref_name in [
        ...
        if os.path.exists(ref_path):
            ...
            print(f"  NCC {label} (WebGPU vs OpenCL ref): {ncc_ref:.4f}")
    ...
    save_nifti(al_unpacked, mni_ref_path,
               os.path.join(WEBGPU_OUT_DIR, f't1_mni_{res}_aligned_linear.nii.gz'))
    save_nifti(anl_unpacked, mni_ref_path,
               os.path.join(WEBGPU_OUT_DIR, f't1_mni_{res}_aligned_nonlinear.nii.gz'))
    ss_unpacked = unpack_volume(ss, mni_data.shape)
    save_nifti(ss_unpacked, mni_ref_path,
               os.path.join(WEBGPU_OUT_DIR, f't1_mni_{res}_skullstripped.nii.gz'))
    interp_unpacked = unpack_volume(interp, mni_data.shape)
    save_nifti(interp_unpacked, mni_ref_path,
               os.path.join(WEBGPU_OUT_DIR, f't1_mni_{res}_interpolated.nii.gz'))
    np.savetxt(os.path.join(WEBGPU_OUT_DIR, f't1_mni_{res}_params.txt'), params)
    if dx.size > 0:
        for comp, name in [(dx, 'x'), (dy, 'y'), (dz, 'z')]:
            comp_unpacked = unpack_volume(comp, mni_data.shape)
            save_nifti(comp_unpacked, mni_ref_path,
                       os.path.join(WEBGPU_OUT_DIR, f't1_mni_{res}_disp_{name}.nii.gz'))
...
if __name__ == '__main__':
    print("WebGPU Registration Validation")
